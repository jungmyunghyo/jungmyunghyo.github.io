...
let kakaoMapUtil = {
	...
	fn_get				: function() {
		...
		var len		= (args.length);
		var id		= (args[0]);
		var h		= ((len > 1) ? (args[1]) : (window.innerHeight));
		var w		= ((len > 2) ? (args[2]) : (window.innerWidth));
		var z		= ((len > 3) ? (args[3]) : ("0"));
		$("#" + (id)).empty();
		var ops = {
		...
		setTimeout(() => kakaoMapUtil.fn_resz(id, h, w, z), 500);
	...
	fn_gps				: function(fn_y, fn_n) {
		...
		if (f) {
			...
			navigator.geolocation.getCurrentPosition(fn_y, fn_n, ops);
		} else {
			fn_n();
		}
	},
...
let kakaoClstrMapUtil = {
	...
	fn_get				: function(id) {
		...
		kakaoClstrMapUtil._map = (new kakao.maps.Map(document.getElementById(id), ops));
		setTimeout(() => kakaoMapUtil.fn_resz(id, h, w, z), 500);
	...
	fn_clstr			: function() {
		var ops = {
			...
			averageCenter	: (true),
			minLevel		: (kakaoClstrMapUtil._clstr_lv)
		};
		kakaoClstrMapUtil._clstr = (new kakao.maps.MarkerClusterer(ops));
	},
	fn_gps_y			: function(position) {
		if (position != null) {
	...
	fn_del_mrk			: function() {
		...
		((kakaoClstrMapUtil._clstr != null) ? (kakaoClstrMapUtil._clstr.clear()) : ("")); 
	...
	fn_add_mrk			: function() {
		...
		((kakaoClstrMapUtil._clstr != null) ? (kakaoClstrMapUtil._clstr.addMarkers(kakaoClstrMapUtil._mrk)) : (""));
...
let kakaoStaticMapUtil = {
	...
	fn_get				: function() {
		...
		var lat		= (args[1]);
		var lot		= (args[2]);
		var h		= ((len > 3) ? (args[3]) : (window.innerHeight));
		var w		= ((len > 4) ? (args[4]) : (window.innerWidth));
		var z		= ((len > 5) ? (args[5]) : ("0"));
		$("#" + (id)).empty();
		$("#" + (id)).css("height", h);
		$("#" + (id)).css("width", w);
		$("#" + (id)).css("z-index", z);
