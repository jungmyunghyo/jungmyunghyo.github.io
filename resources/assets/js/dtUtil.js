const dtUtil = {
	...
	fn_get_mm			: function() {
		...
		var len			= (args.length);
		return (dtUtil.fn_fmt(dtUtil.fn_get_m(((len > 0) ? (args[0]) : (0)), ((len > 1) ? (args[1]) : (0)), ((len > 2) ? (args[2]) : (0)), ((len > 3) ? (args[3]) : (0)), ((len > 4) ? (args[4]) : (0)))));
	},
	fn_get_d			: function() {
	...
	fn_get_dd			: function() {
		...
		var len			= (args.length);
		return (dtUtil.fn_fmt(dtUtil.fn_get_d(((len > 0) ? (args[0]) : (0)), ((len > 1) ? (args[1]) : (0)), ((len > 2) ? (args[2]) : (0)), ((len > 3) ? (args[3]) : (0)))));
	},
	fn_get_h			: function() {
	...
	fn_get_hh			: function() {
		...
		var len			= (args.length);
		return (dtUtil.fn_fmt(dtUtil.fn_get_h(((len > 0) ? (args[0]) : (0)), ((len > 1) ? (args[1]) : (0)), ((len > 2) ? (args[2]) : (0)))));
	},
	fn_get_i			: function() {
	...
	fn_get_ii			: function() {
		...
		var len			= (args.length);
		return (dtUtil.fn_fmt(dtUtil.fn_get_i(((len > 0) ? (args[0]) : (0)), ((len > 1) ? (args[1]) : (0)))));
	},
	fn_get_s			: function() {
	...
	fn_get_ss			: function() {
		...
		var len			= (args.length);
		return (dtUtil.fn_fmt(dtUtil.fn_get_s(((len > 0) ? (args[0]) : (0)))));
	},
	fn_get_ymd			: function() {
		...
		((len > 4 && args[4] > 0) ? (dt.setMinutes(		dt.getMinutes()		+ args[4])) : (""));
		((len > 5 && args[5] > 0) ? (dt.setSeconds(		dt.getSeconds()		+ args[5])) : (""));
		rst				= ((rst) + (dt.getFullYear()));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getMonth() + 1)));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getDate())));
		return (rst);
	},
	fn_get_his			: function() {
		...
		((len > 1 && args[1] > 0) ? (dt.setMinutes(		dt.getMinutes()		+ args[1])) : (""));
		((len > 2 && args[2] > 0) ? (dt.setSeconds(		dt.getSeconds()		+ args[2])) : (""));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getHours())));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getMinutes())));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getSeconds())));
		return (rst);
	},
	...
	fn_fmt_ymd			: function() {
		...
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getMonth() + 1)) + ((tp == "kor") ? ("월") : ((tp == "db") ? ("-") : (tp))));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getDate())) + ((tp == "kor") ? ("일") : ("")));
		return (rst);
	},
	fn_fmt_his			: function() {
		...
		((len > 2 && args[2] > 0) ? (dt.setMinutes(		dt.getMinutes()		+ args[2])) : (""));
		((len > 3 && args[3] > 0) ? (dt.setSeconds(		dt.getSeconds()		+ args[3])) : (""));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getHours())) + ((tp == "kor") ? ("시") : ((tp == "db") ? (":") : (tp))));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getMinutes())) + ((tp == "kor") ? ("분") : ((tp == "db") ? (":") : (tp))));
		rst				= ((rst) + (dtUtil.fn_fmt(dt.getSeconds())) + ((tp == "kor") ? ("초") : ("")));
		return (rst);
	},
	...
	fn_prs				: function(dt) {
		...
		var len			= (v.length);
		return ((v.substring(0, 4)) + ("-") + ((len <= 4) ? ("01") : (v.substring(4, 6))) + ("-") + ((len <= 6) ? ("01") : (v.substring(6, 8))) + (" ") + ((len <= 8) ? ("00") : (v.substring(8, 10))) + (":") + ((len <= 10) ? ("00") : (v.substring(10, 12))) + (":") + ((len <= 12) ? ("00") : (v.substring(12, 14))));
	},
	fn_diff				: function() {
		...
		if (tp == "d") {
			...
			return ((h) + (":") + (dtUtil.fn_fmt(i)) + (":") + (dtUtil.fn_fmt(s)));
		} else if (tp == "dhis") {
			...
			return ((d) + (":") + (dtUtil.fn_fmt(h)) + (":") + (dtUtil.fn_fmt(i)) + (":") + (dtUtil.fn_fmt(s)));
		}
	}
};
const dtCalUtil = {
